# Workflow Summary
This document records the implementation and reproducibility details for `memd` as represented in the repository state on branch `main`, version `0.3.0`. `memd` is a local Model Context Protocol (MCP) server for shared knowledge artifacts. The software combines append-only persistent storage, SQLite metadata, Candle-based dense embeddings, HNSW approximate nearest-neighbor search, Tantivy BM25 sparse retrieval, hybrid fusion and reranking, a task-oriented knowledge artifact layer, canonical artifact exchange APIs, summary-first digest artifacts with dedicated briefing/library MCP tools, explicit trust-boundary metadata and grounding APIs, structural code indexing, and optional feature-gated ONNX cross-encoder reranking. This Methods record is grounded in checked-in code, configuration, tool definitions, and benchmark artifacts, and it avoids claims that are not supported by repository evidence.

## Software Artifact and Evidence Package
The software artifact audited for this Methods record is the Rust workspace declared in `Cargo.toml` and the `memd` crate declared in `crates/memd/Cargo.toml`. The repository remote recorded for the audited artifact is `https://github.com/fmschulz/memd.git`. The package version is `0.3.0`, the workspace edition is Rust 2021, and the primary implementation modules are exposed through `crates/memd/src/lib.rs`. The evidence package used to prepare this document consisted of the top-level user documentation in `README.md` and `QUICKSTART.md`, the default configuration in `configs/default.toml`, the MCP tool schema in `crates/memd/src/mcp/tools.rs`, the persistent storage and retrieval code in `crates/memd/src/store/*` and `crates/memd/src/index/*`, the task-memory schema in `crates/memd/src/task_memory/mod.rs` and `docs/scientific-task-memory/schema/README.md`, and the checked-in benchmark protocols and reports in `evals/BENCHMARK_PROTOCOL.md` and `docs/scientific-task-memory/benchmark-results/`.
...
`memd` exposes its functionality through an MCP server and a local CLI. The current MCP tool surface contains 41 tools distributed across generic memory operations, task-oriented artifact operations, canonical artifact operations, context retrieval, structural code analysis, and debugging queries. The top-level architectural split is between raw searchable memory written with `memory.*` tools, structured task knowledge written with `task.*` tools, and direct artifact exchange written and read through `artifact.*` tools. This distinction is fundamental to the software design and is not merely an interface convenience: `memory.*` stores flexible chunks, `task.*` stores canonical lifecycle artifacts and projects them into retrieval-optimized chunks, and `artifact.*` exposes the canonical artifact layer directly for critique, revision, verification, and thread inspection. The checked-in tool surface now also includes `task.resume`, `context.brief_project`, the library-style helpers `artifact.find_failures`, `artifact.find_decisions`, `artifact.find_evidence`, and `artifact.find_highlights`, and the explicit grounding tool `artifact.verify`.
...
The checked-in skill package also includes an enforcement installer that upserts explicit `memd`-usage instructions into `~/.codex/AGENTS.md` and `~/.claude/CLAUDE.md`. This does not harden the clients at the binary level, but it is the strongest practical instruction-layer mechanism available in the repository for making `memd` usage habitual and expected in substantive sessions.
...
The task-oriented knowledge layer keeps a canonical task artifact envelope separate from retrieval projections. The canonical envelope captures fields such as task goal, motivation, hypothesis, scientific question, tool choice, parameters, inputs, outputs, evidence, validation, uncertainty, follow-up actions, promotion state, digest identity, and source freshness timestamps. Canonical artifact kinds now include `decision` and `digest` in addition to the lifecycle kinds `task_start`, `task_progress`, `run_start`, `run_finish`, `evidence`, `review`, `revision`, `verification`, and `task_finish`. These canonical artifacts are projected into ordinary retrieval chunks of kinds such as `task_goal`, `task_summary`, `run`, `evidence`, `decision`, `digest`, `worked`, `failed`, and `validation`. Exact task-aware filters are applied through normalized SQLite side tables before the candidate set is reranked for `task.search`. This design is explicitly documented in the checked-in task schema README and benchmarked in the task-memory evaluation artifacts.
...
At query time, `memory.search` and the broader retrieval path first obtain candidate results through hybrid dense+sparse search when the persistent hybrid searcher is available. Dense retrieval embeds the query with Candle, sparse retrieval performs BM25 search over the lexical index, and the resulting candidate set is fused. Full reranking occurs after candidate chunks are fetched and annotated with metadata and text content. If the service was started with the default `hybrid-feature` variant, the feature reranker is used. If the service was started with `hybrid-cross-encoder` and the feature flag plus ONNX runtime are available, the ONNX cross-encoder is used in the reranking step. In-memory mode bypasses these persistent retrieval backends and uses a simple in-memory search path, which is suitable for smoke testing but not for evaluating the full retrieval architecture. The search surfaces `memory.search`, `task.search`, and `artifact.search` also accept optional query modes (`generic`, `brief_project`, `resume_task`, `find_failures`, `find_decisions`, and `find_evidence`) that bias candidate planning toward persisted briefs or canonical task/artifact summaries instead of only raw chunk retrieval.

`task.search` follows a distinct query plan. It first resolves exact candidate chunk identifiers from normalized task tables using the requested filters, then reranks that candidate set for the query text. Dedicated MCP helpers such as `context.brief_project`, `task.resume`, `artifact.find_failures`, `artifact.find_decisions`, and `artifact.find_evidence` build on the same canonical state by generating or refreshing persisted digest artifacts on demand. The `memory.compact` handler can also regenerate project brief and failure/decision/evidence library digests when `project_id`, `digest_modes`, or `force_digest_rebuild` are supplied. The checked-in benchmark results are consistent with this design: the structured task-memory path incurs more write-time expansion but yields substantially better artifact recovery than flattened chunk search on the hardened Phase 5 corpus.
