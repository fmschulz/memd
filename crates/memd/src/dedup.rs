...
use crate::ops::{error::OperationError, DedupConfig, DedupSpec};
...
pub fn compute_dedup_candidates(
    ...
    cfg: &ResolvedDedup,
) -> std::result::Result<Vec<ChunkId>, OperationError> {
    ...
    let candidates = match cfg.mode {
        DedupMode::Exact => ps
            ...
            .map_err(|e| OperationError::ToolError(e.to_string()))?,
...
fn fuzzy_candidates(
    ...
    cfg: &ResolvedDedup,
    project_id: Option<&str>,
) -> std::result::Result<Vec<ChunkMetadata>, OperationError> {
